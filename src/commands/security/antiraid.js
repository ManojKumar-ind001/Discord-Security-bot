...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (sub === 'on') {
      ...
      return interaction.reply(V2.reply(V2.success('Anti-Raid Enabled', '🛡️ Anti-raid protection is now **ON**.', client)));
    }
    if (sub === 'off') {
      data.security.antiRaid = false;
      await GuildModel.save(interaction.guild.id, data);
      return interaction.reply(V2.reply(V2.warning('Anti-Raid Disabled', '⚠️ Anti-raid protection is now **OFF**.', client)));
    }
    if (sub === 'unlock') {
      ...
      await interaction.editReply(V2.reply(V2.success('Channels Unlocked',
        `🔓 Unlocked **${n} channels** after raid.`, client)));
    }
  },
};
